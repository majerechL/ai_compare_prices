...
def main():
    ...
    streamlit.title("Porovnavanie cien")

    with streamlit.form(key="user_input_form"):
        product_input = streamlit.text_input("Zadaj produkt, ktorého cenu chceš porovnať: ")
        submit_button = streamlit.form_submit_button(label="Potvrdiť")

        if submit_button:

            if "assistant" not in streamlit.session_state: # ak neexistuje asistent, tak ho vytvorí
                assistant = client.beta.assistants.create(name="Porovnávanie cien", # tvorba OpenAI Assistenta s menom, modelom, inštrukciami zo súboru, nástrojmi
                    model=model,
            ...
            if "thread" not in streamlit.session_state:
                ...
                streamlit.session_state["thread"] = thread # uloženie do session state

            manager = AssistantManager(
                client, streamlit.session_state["assistant"],
                streamlit.session_state["thread"]) # vytvorenie managera s asistentom a threadom

            manager.add_message_to_thread(role="user", content=product_input)
